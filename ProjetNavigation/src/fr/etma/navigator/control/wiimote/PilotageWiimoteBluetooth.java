...
public class PilotageWiimoteBluetooth extends JFrame {
   ...
   public PilotageWiimoteBluetooth (final Navigator navigator) {
      ...
      bluetoothField = new JTextField ("0021BD776F49") ;
		...
		public void IRInputReceived (WRIREvent ire) {
		   if (wiimoteDirectionActivated) {
		      ...
		      if ((ire.getIRLights () [0] != null) && (ire.getIRLights () [1] != null)) {
		         ...
		         deltaX = ire.getIRLights () [0].getX () - ire.getIRLights () [1].getX () ;
		         deltaY = ire.getIRLights () [0].getY () - ire.getIRLights () [1].getY () ;
		         ...
		         double pitch  ;
		         pitch = Math.atan2 (y - 0.5, 0.1 / sensorBarLenght) ; 
		         pitchs.addFirst (pitch);
		         if (pitchs.size () > historyLenght) {
		        	 pitchs.removeLast () ;
		         }
		         double roll ;
		         roll = Math.asin (-deltaY / sensorBarLenght) ; 
		         rolls.addFirst (roll);
